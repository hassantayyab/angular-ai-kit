  ...
  template: `
    <div hlmInputGroup class="[--radius:1.2rem]">
                ...
                @for (groupType of _groupTypes(); track groupType) {
                  <hlm-command-group [id]="groupType">
                    <hlm-command-group-label>{{
                      groupType === 'page' ? 'Pages' : 'Users'
                    }}</hlm-command-group-label>
                    @for (item of _grouped()[groupType]; track item.title) {
                      <button
                        hlm-command-item
                        [value]="item.title"
                        (click)="addMention(item); ctx.close()"
                      >
      ...
      <div
        hlmInputGroupAddon
        align="block-end"
        class="no-scrollbar gap-1 overflow-y-auto"
      >
          ...
          <hlm-dropdown-menu class="w-50 [--radius:1rem]">
            <hlm-dropdown-menu-group>
              <hlm-dropdown-menu-label class="text-muted-foreground text-xs"
                >Select Agent Mode</hlm-dropdown-menu-label
              >
        ...
        <button
          ...
          class="rounded-full"
          size="sm"
          [hlmDropdownMenuTrigger]="sources"
        >
...
export class NotionPrompt {
  ...
  protected _mentionButtonSize = computed(() =>
    this._hasMention() ? 'icon-sm' : 'sm'
  );
  ...
  removeMention(item: MentionItem) {
    this._mentions.update((mentions) =>
      mentions.filter((mention) => mention.title !== item.title)
    );
  }
